	...
	initGrid = () => {
		...
		for(let row = 0; row < 9; row++ ) {
			...
			for(let col = 0; col < 9; col++ ) {
				...
				// 	currentRow.push(Math.floor(Math.random() * 9 + 1));
				// } else {
				// 	currentRow.push('');
				// }
				currentRow.push('');
	...
	render() {
		return (
			<div className={classes.InputGrid}>
				...
				<ul>
					<li><Button variant={"contained"} color={"primary"} onClick={() => this.handleModal(true)}>Solve</Button></li>
					<li><Button variant={"contained"} color={"secondary"} onClick={this.handleReset}>Reset</Button></li>
				</ul>
				<Modal
				...
				>
					<Fade in={this.state.showConfirmation}>
